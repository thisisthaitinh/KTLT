...
void TableDisplay(SinhVien *a, int n)
{
     ...
     for (i = 0; i < n; ++i)
     {
          printf("%d\t%s\t%d\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\t%s", i + 1, a[i].name, a[i].number, a[i].diem_tk, a[i].diem_gk, a[i].diem_ck, a[i].diem_th, a[i].diem_tb, StatusDisplay(a));
          printf("\n");
     }
}
...
int main()
{
     int x = malloc(n * sizeof(SinhVien));
     system("cls");
     int choice;
     do
     {
